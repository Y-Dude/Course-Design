...
var Lr1ParserVis = function (document) {
  ...
  this.renderCollectionVizDot = container => {
    container.innerHTML = '';
    container.innerHTML = Viz(getCollectionVizDot(getCollectionViz(_collection),getGotoEges(_parseTable)));
  }
  ...
  const getCollectionVizDot = (states, edges) => {
    ...
    for (let key in states) {
      let stateString = "";
      states[key].forEach((line) => {
        stateString += line["left"] + '->' + line["right"].join("") + '\n';
      });
      dot += `"${key}" [ label ="${key}:\n${stateString}" ];\n`;
    }


    for (let state in edges) {
      for (let edge in edges[state]) {
        edgeString += 'I' + state + ' -> I' + edges[state][edge] + ' [ label="' + edge + '" ];';
      }
    }
    dot += edgeString + '}';
    return dot;
  ...
  const getCollectionViz = (collection) => {
    let collectionVizObj = {};
    collection.forEach((state, sIndex) => {
      let statekey = `I${sIndex}`;
      collectionVizObj[statekey] = [];
      state.forEach((line, lineIndex) => {
        collectionVizObj[statekey][lineIndex] = {};
        collectionVizObj[statekey][lineIndex]["left"] = _symbols[line.rule.lhs];
        collectionVizObj[statekey][lineIndex]["right"] = [];
        line.rule.rhs.forEach(rSymbol => {
          collectionVizObj[statekey][lineIndex]["right"].push(_symbols[rSymbol]);
        });

        collectionVizObj[statekey][lineIndex]["right"].push(",");
        collectionVizObj[statekey][lineIndex]["right"].push(_symbols[line.lookahead]);
        collectionVizObj[statekey][lineIndex]["right"].splice(line.position, 0, "·");

      });
    });
    return collectionVizObj;
  }

  const getGotoEges = (parsetable) => {
    let gotoEdges = {};
    let flage = true;
    parsetable.forEach((line, state) => {
      line.forEach((toState, edgeIndex) => {
        if (toState.type === "shift" || typeof toState !== "object") {
          if (flage) {
            gotoEdges[state] = {};
          }
          flage = false;
          gotoEdges[state][_symbols[edgeIndex]] = typeof toState === 'object' ? toState.state : toState;
        }
      });
      flage = true;
    });
